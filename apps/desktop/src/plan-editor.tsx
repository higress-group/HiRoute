...
export const PlanEditor = forwardRef<PlanEditorHandle, { plan?: Plan; draft?: Draft; creating?: boolean; initialBindingId?: string; language: 'zh' | 'en'; active?: boolean; usedBy?: { id: string; name: string; brand: 'codex' | 'claude-code' | 'agent'; isDefault: boolean }[]; onOpenAgent?: (agentId: string) => void; onOpenSession?: (sessionId: string, requestId: string) => void; onDone: () => Promise<void>; onClose: () => void; onDirty?: (dirty: boolean) => void; onEdit?: () => void; onBusyChange?: (busy: boolean) => void; onOperation?: (operation: PlanOperation | null) => void; onPersisted?: (editor: PersistedEditor<Plan, Draft> | null, action: 'save_draft' | 'publish', identity: PersistenceIdentity, operation: PlanOperation | null) => void }>(function PlanEditor({ plan, draft, creating = false, initialBindingId, language, active = true, usedBy = [], onOpenAgent, onOpenSession, onDone, onClose, onDirty, onEdit, onBusyChange, onOperation, onPersisted }, ref) {
      ...
      if (issue) {
        ...
        requestAnimationFrame(() => {
          if (issue.field === 'alias') {
            ...
            input?.focus();
          } else if (issue.field === 'context_window') {
            document.querySelector<HTMLInputElement>('.plan-context-window-field')?.focus();
          } else if (issue.field === 'request_timeout') {
            document.querySelector<HTMLInputElement>('.plan-request-timeout-field')?.focus();
  ...
  return <section className="plan-editor" aria-label={text('路由编辑器', 'Plan editor')}>{!creating && <header className="editor-header"><div className="editor-title"><div className="title-with-status"><h2>{editor.display_name || text('新建智能路由', 'New smart routing')}</h2><span className={`badge ${editorTone} no-dot`}>{editorState}</span></div></div><div className="editor-actions"><button className="btn" type="button" disabled={busy || (!dirty && Boolean(plan || draft))} onClick={() => void act('save_draft')}>{text('保存草稿', 'Save draft')}</button><button type="button" className="btn btn-primary" disabled={busy || (!dirty && !!plan && base.expected_draft_revision === null)} onClick={() => void act('publish')}><UiIcon name="upload" />{plan ? text('发布更改', 'Publish changes') : text('启用', 'Enable')}</button></div></header>}
    ...
    <section className="editor-section" data-plan-context-window>
      <div className="editor-section-heading"><div><h3>{text('上下文窗口', 'Context window')}</h3><p>{text('默认最多使用 272,000 tokens；可在候选共同上界内调整。较大的窗口可能增加上下文与缓存费用。', 'Defaults to at most 272,000 tokens. Adjust within the shared candidate limit. Larger windows may increase context and cache costs.')}</p></div></div>
      <label className="field"><span className="field-label">{text('自定义窗口 · tokens（留空使用默认值）', 'Custom window · tokens (leave empty for default)')}</span>
        <input className="input plan-context-window-field" type="number" min="1" step="1" max={options?.context_window?.maximum_tokens} placeholder={String(options?.context_window?.default_tokens ?? 272000)} value={editor.limits.context_window_tokens ?? ''} aria-invalid={Boolean(!timeoutValidationError && editor.limits.context_window_tokens !== undefined && contextWindowError(editor.limits.context_window_tokens, options?.context_window, language))} onChange={event => update({ limits: { ...editor.limits, context_window_tokens: event.target.value === '' ? undefined : Number(event.target.value) } })} />
      </label>
      <p className="field-help">{text('已接入的 Agent 需重新保存模型设置并重新启动，才能加载更新后的窗口；进行中的会话不会立即改变。', 'For an existing Codex connection, save the Agent model settings again and restart Codex to load the updated window. Running sessions do not change immediately.')}</p>
      <button type="button" className="btn" disabled={editor.limits.context_window_tokens === undefined} onClick={() => update({ limits: { ...editor.limits, context_window_tokens: undefined } })}>{text('恢复默认', 'Restore default')}</button>
      <p className="field-help">{options?.context_window ? text(`默认 ${options.context_window.default_tokens.toLocaleString()} tokens · 候选共同上界 ${options.context_window.maximum_tokens.toLocaleString()} tokens`, `Default ${options.context_window.default_tokens.toLocaleString()} tokens · Shared candidate limit ${options.context_window.maximum_tokens.toLocaleString()} tokens`) : text('完成候选配置后显示可用上界。', 'Complete the candidate configuration to see the available limit.')}</p>
      {!timeoutValidationError && editor.limits.context_window_tokens !== undefined && contextWindowError(editor.limits.context_window_tokens, options?.context_window, language) && <p className="oc-inline-error" role="alert">{contextWindowError(editor.limits.context_window_tokens, options?.context_window, language)}</p>}
    </section>
    <section className="editor-section" data-plan-request-timeout>
      <div className="editor-section-heading"><div><h3>{text('单次请求最长等待', 'Maximum wait per request')}</h3><p>{text('包含请求内重试和流式输出；超过时限会中断仍在输出的回复。新计划默认 1 小时。', 'Includes retries and streaming output. A response still streaming at the limit is interrupted. New plans default to 1 hour.')}</p></div></div>
      <label className="field"><span className="field-label">{text('等待时间（秒）', 'Wait time (seconds)')}</span>
        <input className="input plan-request-timeout-field" type="number" min={Math.ceil(editor.limits.attempt_timeout_ms / 1000)} max={3600} step={1} value={editor.limits.request_timeout_ms / 1000} aria-invalid={Boolean(timeoutValidationError)} onChange={event => update({ limits: { ...editor.limits, request_timeout_ms: event.target.value === '' ? 0 : Number(event.target.value) * 1000 } })} />
      </label>
      {timeoutValidationError && <p className="oc-inline-error" role="alert">{timeoutValidationError}</p>}
    </section>
    <section className="editor-section"><label className="field-label">{text('目标 Agent 能力预览', 'Target Agent capability preview')}<select className="select" value={previewAgent} onChange={event => setPreviewAgent(event.target.value as 'codex' | 'claude')}><option value="codex">Codex</option><option value="claude">Claude Code</option></select></label></section>
    {previewAgent === 'claude' && <section className="editor-section"><h3>Claude Code</h3>{options?.claude_capabilities?.state === 'available' ? <><p>{text(`自动压缩窗口最多 ${formatContext(options.claude_capabilities.context_window)} tokens`, `Auto-compact window up to ${formatContext(options.claude_capabilities.context_window)} tokens`)}</p><p className="field-help">{text('作用于整个 Claude Code 进程；接入多个计划时取最小窗口，原生模型可能更早压缩。接入时会核对窗口、配置冲突和请求能力。', 'Applies to the whole Claude Code process. Multiple plans share the smallest window; native models may compact earlier. Connection checks window constraints, configuration conflicts and request capabilities.')}</p></> : <p className="field-help">{options?.claude_capabilities?.state === 'unavailable' && options.claude_capabilities.reason === 'context_window_below_minimum' ? text('Claude Code 自动压缩窗口最低为 100K；请调整计划窗口或使用 Codex。', 'Claude Code requires an auto-compact window of at least 100K. Adjust the plan window or use Codex.') : text('当前候选或窗口尚不能用于 Claude Code；需要可用的 Messages 入口和有效窗口。', 'The current candidates or window are unavailable for Claude Code. A usable Messages ingress and valid window are required.')}</p>}</section>}
    {previewAgent === 'codex' && codexCapabilities && <section className="editor-section codex-capability-summary" data-codex-capability-state={codexCapabilities.state}><div className="editor-section-heading"><div><h3>{text('对 Codex 的能力预览', 'Codex capability preview')}</h3><p>{text('依据当前编辑内容及候选的 Codex Responses 入口能力计算。上游使用其他协议时仍可能通过 HiRoute 适配；发布和接入时会重新核对，预览不代表真实客户端验证。', 'Calculated from the current edit and candidate Codex Responses ingress capabilities. HiRoute may adapt another upstream protocol; publication and connection recheck compatibility, and this preview is not a live client verification.')}</p></div></div>{codexCapabilities.state === 'available' ? <div className="option-panel"><div className="option-row" data-codex-capability-summary><div><strong>{text(`上下文 ${formatContext(codexCapabilities.context_window)} · 输入 ${codexCapabilities.input_modalities.includes('image') ? '文本/图片' : '文本'}`, `Context ${formatContext(codexCapabilities.context_window)} · Input ${codexCapabilities.input_modalities.includes('image') ? 'text/images' : 'text'}`)}</strong><span>{text('推理强度由路由配置决定', 'Reasoning effort is determined by the route configuration')}</span></div></div>{codexCapabilities.limitations.map(limit => <div className="callout warn" role="status" data-codex-capability-limit={limit.kind} key={limit.kind}><UiIcon name="warning" /><div><strong>{limit.kind === 'context_window' ? text('候选收窄了共同上下文', 'A candidate narrows the shared context') : text('候选收窄了图片输入', 'A candidate narrows image input')}</strong><p>{limit.kind === 'context_window' ? text(`${capabilityBindings(limit.binding_ids)} 将共同上下文限制为 ${formatContext(options?.context_window?.maximum_tokens ?? codexCapabilities.context_window)}。`, `${capabilityBindings(limit.binding_ids)} limits the shared context to ${formatContext(options?.context_window?.maximum_tokens ?? codexCapabilities.context_window)}.`) : text(`${capabilityBindings(limit.binding_ids)} 不支持完整的 Codex 图片输入；最终目录只声明文本。`, `${capabilityBindings(limit.binding_ids)} does not support complete Codex image input, so the final catalog declares text only.`)}</p></div></div>)}{codexCapabilities.fixed_limits.includes('parallel_tool_calls_disabled') && <div className="callout" data-codex-fixed-limit="parallel_tool_calls_disabled"><UiIcon name="info" /><div><strong>{text('HiRoute 当前固定使用串行工具调用', 'HiRoute currently uses serial tool calls')}</strong><p>{text('这是当前实现限制，不由任何候选模型造成。', 'This is a fixed implementation limit, not a candidate-model limitation.')}</p></div></div>}</div> : <div className="callout bad" role="alert" data-codex-capability-unavailable><UiIcon name="warning" /><div><strong>{text('无法生成可靠的 Codex 客户端能力', 'Reliable Codex client capabilities are unavailable')}</strong>{codexCapabilities.issues.map((issue, index) => <p key={`${issue.kind}:${issue.binding_id ?? index}`}>{capabilityIssue(issue)}</p>)}</div></div>}</section>}
