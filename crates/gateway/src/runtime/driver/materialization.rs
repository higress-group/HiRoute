...
use super::{
    ...
    MATERIALIZATION_PROTOCOL_FAILED, MechanicalFailureSignal, PermitAvailability,
    ProductionAttemptState, ProductionLogicalRequest, ProductionProvider,
...
pub(super) async fn materialize_attempt(
    ...
    let renderer = (streaming && !native_output)
        .then(|| {
            ...
            .map(|renderer| renderer.with_budget(context.budget.clone()))
        })
        .transpose()
        .map_err(|_| Arc::from(MATERIALIZATION_PROTOCOL_FAILED))?;
